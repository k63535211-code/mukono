import { api, type DeviceKind, type HealthStatus, type ManagedDevice, type MonitorAlert, type NetworkTarget, type Overview } from './api';
import './styles.css';
...
function renderDevices(): string {
  ...
  return `
    <section class="section-heading">
      <div><div class="eyebrow accent">Managed estate</div><h2>Enrolled devices</h2><p>Agent-reported health and resource usage.</p></div>
    ...
    <section class="panel table-panel">
      ...
      <div class="table-scroll"><table><thead><tr><th>Device</th><th>Type</th><th>Address</th><th>Status</th><th>CPU</th><th>Memory</th><th>Disk</th><th>Last seen</th></tr></thead><tbody>${filteredDevices.length ? filteredDevices.map(deviceRow).join('') : emptyRow(8, state.devices.length ? 'No devices match the current filters.' : 'No managed devices have reported in yet.')}</tbody></table></div>
    </section>
